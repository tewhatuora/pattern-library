...
export const PropsTable = ({ sourceLink, types }: Props) => {
  ...
  return (
    <>
      {props.length ? (
        <Box overflow={{ mobile: 'scroll' }} width="full">
          <Box as="table" width="full">
            <Box as="thead">
              <Box as="tr" textAlign="left">
                {headers.map((x, i) => (
                  <Box as="th" backgroundColor="neutral5" key={x} position="sticky" top={0}>
                    <Box
                      backgroundColor="neutral5"
                      borderColor="neutral75"
            ...
            <Box as="tbody">
              {props.map((x) => (
                <Box as="tr" borderBottomWidth="standard" key={x.name}>
                  <Box {...dataProps}>
                    <Text color="neutral75" size="small">
                      {x.name}
                      {x.required && (
                        <Text as="span" color="error100" size="small">
                          *<ScreenReadersOnly>Required</ScreenReadersOnly>
                        </Text>
                      )}
                    </Text>
                  </Box>
        ...
        <Box>
          <Text color="neutral75">No props</Text>
        </Box>
      )}

      <Box marginY={2}>
        <Stack align="right" space={2}>
          {!!props.length && (
            <Button
              variant="tertiary"
              onClick={() =>
